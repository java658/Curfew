...
public interface DetailsRepo extends JpaRepository<Details, Integer> {
    ...
    @Query(value = "SELECT * FROM user_data where status='requested' ", nativeQuery = true)
    List<Details> findAllRequests();
    
    @Query(value = "SELECT * FROM user_data where status='processing' ", nativeQuery = true)
    List<Details> findAllProcessing();
    
    @Query(value = "SELECT * FROM user_data where status='finalStage' ", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE user_data SET status='processing' WHERE id=?1 ", nativeQuery = true)
    public void updateStatusToProcessing(Integer id);
    
    @Modifying
    @Transactional
    @Query(value = "UPDATE user_data SET status='finalStage' WHERE id=?1 ", nativeQuery = true)
    public void updateProcessingToFinalStage(Integer id);
    
    @Modifying
    @Transactional
    @Query(value = "UPDATE user_data SET status='success' WHERE id=?1 ", nativeQuery = true)
